...
public:
  ...
  BigInt(const std::vector<uint64_t> &d, bool neg = false)
      : digits(d), is_negative(neg) {
  ...
  BigInt operator+(const BigInt &other) const {
    ...
    return (*this < other) ? sub_abs(other, *this).negate()
                           : sub_abs(*this, other);
  }
  ...
  std::vector<uint8_t> toBytes() const {
    ...
    output.reserve(digits_.size() * 8);
    for (uint64_t value : digits_) {
      for (int i = 0; i < 8; ++i) {
        output.push_back(static_cast<uint8_t>((value >> (56 - i * 8)) & 0xFF));
      }
    }

    int count = 0;
    while (output.size() > 0 && output[count] == 0) {
      count++;
    }
  ...
  static BigInt BytesToBigInt(const std::vector<uint8_t> &input) {
    ...
    for (size_t i = 0; i < input.size(); i += 8) {
      ...
      for (int j = 0; j < 8; ++j) {
        ...
        const uint8_t byte =
            (byte_index < input.size()) ? input[byte_index] : 0;
...
private:
  ...
  static BigInt sub_abs(const BigInt &a, const BigInt &b) {
    ...
    for (size_t i = 0; i < res.digits.size(); ++i) {
      int64_t diff =
          res.digits[i] - borrow - (i < b.digits.size() ? b.digits[i] : 0);
      if (diff < 0) {
        diff += BASE;
        borrow = 1;
      } else {
  ...
  static BigInt naive_mult(const BigInt &a, const BigInt &b) {
    ...
    for (size_t i = 0; i < a.digits.size(); ++i) {
      ...
      for (size_t j = 0; j < b.digits.size() || carry; ++j) {
        __uint128_t cur =
            res.digits[i + j] +
            (__uint128_t)a.digits[i] * (j < b.digits.size() ? b.digits[j] : 0) +
            carry;
