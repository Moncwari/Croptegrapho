...
def MixColumns(block):
    ...
    for i in range(4):
        ...
        for ii in range(4):
            ...
            poly_old_column.append(
                Poly.from_list([int(x) for x in bin_byte[2:]], gens=x)
            )
...
def AES_cipher(message, key, flag):
    ...
    if flag == 0:
        ...
        for i in range(1, Nk + 6):
            block_of_message = XOR(
                MixColumns(ShiftRows(SubBytes(block_of_message))), keys[i]
            )
        ...
        print(tabulate(block_of_message, tablefmt="grid"))
        for i in range(1, Nk + 6):
            block_of_message = InvSubBytes(
                InvShiftRows(InvMixColumns(XOR(block_of_message, keys[i])))
            )
